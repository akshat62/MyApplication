...
class LocalPipelineTest {
    @Test fun realSpeechToPlayableReel() = runBlocking {
        ...
        val speech = TranscriptionEngine(manager, chunkSeconds = 15).transcribe(wav, WhisperModelSpec.BASE)
        assertTrue(speech.isNotEmpty())
        assertTrue("Second chunk must retain source timestamps", speech.any { it.startMs >= 15000 })
